    ...
    <section className='extra'>
      <div className='container extra__container'>
        ...
        <div className='extra__card-list'>
          {productsList.map((item) => item.type === 'extra' && <ExtraCard key={item.article} article={item.article} name={item.name} price={item.price} descriptionList={item.descriptionList} img={item.img} onAddInCart={handleCardSubmit} />)}
        </div>
      </div>
    </section>
  );
