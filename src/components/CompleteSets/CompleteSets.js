    ...
    <section className='complete-sets'>
      ...
      <div className='container complete-sets__container'>
        <div className='complete-sets__card-list'>
          {productsList.map((item) => item.type === 'complete-set' && <CompleteSetCard key={item.id} name={item.article} price={item.price} description={item.description} img={item.img} onAddInCart={handleCardSubmit}/>)}
        </div>
      </div>
    </section>
  );
