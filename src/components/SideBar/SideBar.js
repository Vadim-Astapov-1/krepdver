  ...
  return(
    <div className={`sidebar ${!isHidden ? 'sidebar_visible' : ''}`}>
      <nav className='sidebar__menu'>
        <Link to='/' className={`sidebar__link ${location.pathname === '/' ? 'sidebar__link_active' : ''}`} onClick={handleNavMenuVisible}>Главная</Link>
        <Link to='/cart' className={`sidebar__link ${location.pathname === '/cart' ? 'sidebar__link_active' : ''}`} onClick={handleNavMenuVisible}>Корзина</Link>
